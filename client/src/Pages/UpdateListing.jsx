...
const CreateListing = () => {
    ...
    const handleImageSubmit = () => {
        if(files.length > 0 && files.length + formData.imageUrls.length < 7){
            ...
            Promise.all(promises).then((urls) => {
                ...
                setImageUploadError('Image upload failed (2 mb per image)', err);
                setUploading(false)
    ...
    const handleChange = (e) => {
        ...
        if(e.target.id === 'pricing' || e.target.id === 'furnished' || e.target.id == 'offer'){
            setFormData({
                ...formData, 
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            if(formData.imageUrls.length < 1) return setError("You must upload atleat one image")
            ...
            <div className="flex flex-col gap-4 flex-1">
                ...
                <div className="flex gap-6 flex-wrap mt-4">
                    <div className="flex gap-2">
                        <input type="checkbox" id="sale" className="w-5" onClick={handleChange} checked={formData.type === 'sale'}/>
                        <span>Sell</span>
                    </div>
                    <div className="flex gap-2">
                ...
                <div className="flex flex-wrap gap-6 mt-4">
                    ...
                    {formData.offer && (
                         <div className="flex items-center gap-2">
                         <input type="number" id="disountPrice" min='0' max='10000000' required className="p-3 border border-gray-300 rounded-lg" onChange={handleChange} value={formData.discountPrice}/>
                         <div className="flex flex-col items-center">
